...
class LocationSearch extends GetView<ExploreController> {
  ...
  Widget _buildSearchInput() {
    final TextEditingController textController = TextEditingController(text: controller.searchQuery.value);
  ...
  Widget _buildSearchResultItem(LocationResult result) {
    return InkWell(
      onTap: () {
        try {
          DebugLogger.info('📍 User selected location: ${result.description}');
          controller.selectLocationResult(result);
        } catch (e, stackTrace) {
          DebugLogger.error('❌ Error handling location selection', e, stackTrace);
          Get.snackbar(
            'Error',
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      title: Text(result.displayText ?? result.description),
                      ...
                      onTap: () {
                        try {
                          ...
                          exploreController.selectLocationResult(result);
                          Get.back();
                        } catch (e, stackTrace) {
                          DebugLogger.error('❌ Error handling location selection in modal', e, stackTrace);
                          Get.snackbar(
                            'Error',
                            'Failed to select location',
                            snackPosition: SnackPosition.BOTTOM,
